//
//  EditPlantDetailsViewController.swift
//  GreenThumb
//
//  Created by Sanjukta Roy on 12/19/18.
//  Copyright © 2018 Mana Roy Studio. All rights reserved.
//

import UIKit

/*class EditPlantDetailsViewController: UIViewController {
    var plant: Plant?
    ...
    override func viewDidLoad() {
        ...
        setupDetailsTable()
        setupImagePicker()
        for detail in PlantDataItem.items.keys {
...
extension EditPlantDetailsViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return section == 0 ? PlantDataItem.items.count : 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let dataItem = PlantDataItem.it[indexPath.row]
        let label = "\(dataItem.rawValue): "
        let detail = details[dataItem]
        var cell: UITableViewCell
        if editMode {
            cell = editModeTextCell(label, detail!)
    ...
    private func editModeTextCell(_ label: String, _ detail: String) -> EditPlantTextCell {
...
extension EditPlantDetailsViewController: UIImagePickerControllerDelegate {
